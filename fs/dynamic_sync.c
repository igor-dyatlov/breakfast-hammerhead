...
/*
 * A time slice in milliseconds to wait after LCD turned off before the
 * start of a graceful overall file synchronization. This delay is used
 * only in pair with LCD notifier enabled.
 */
				     ...
				     const char *buf, size_t count)
{
	 ...
	 */
	if (unlikely(!val && delayed_work_pending(&force_sync_work)))
