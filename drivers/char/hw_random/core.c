...
static unsigned short default_quality; /* Default to "off" (0) */
...
static inline size_t rng_buffer_size(void)
{
...
skip_init:
	...
	current_quality =
		min(rng->quality ? rng->quality : default_quality, 1024);

	if (!current_quality && hwrng_fill)
...
void hwrng_unregister(struct hwrng *rng)
{
	...
	if (current_rng == rng) {
		...
		if (!list_empty(&rng_list)) {
			struct hwrng *tail =
				list_entry(rng_list.prev, struct hwrng, list);
