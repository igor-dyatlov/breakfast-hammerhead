 ...
 *
 * Returns per-cpu synchronization information specific to a passed cpu.
 */
 ...
 * synchronized after task migration between them.
 */
static unsigned int boost_ms;
module_param(boost_ms, uint, 0644);

/*
 * Frequency cap for synchronization algorithm.  Shared frequency of
 * synchronized cpus will not go above this threshold if it is set to
 * non-zero value.
 */
...
/*
 * Time in milliseconds to keep frequencies of all online cpus boosted after
 * an input event.  Note that multiple input events, that occured during the
 * time interval which is less or equal to MIN_INPUT_INTERVAL, will be
 * accounted as one.
 */
...
/*
 * Minimum task load that is considered as noticable. If a task load is less
 ...
 * events or only to those which maintain task load more than specified by
