 ...
 *
 * ! Note that this call does not do any bounds checking. Passed values must
 *   be checked on validity by a caller before the usage.
 */
 ...
 *
 * Returns 0 on success or -EINVAL[-EEXIST] if a error occured.
 */
int snd_ctrl_register(struct snd_ctrl_data *snd_data)
{
	...
	if (unlikely(find_ctrl_data(snd_data->name))) {
		pr_err("%s ctrl data already exists\n", snd_data->name);
	...
	pr_debug("%s ctrl data is successfully registered\n", snd_data->name);
...
void snd_ctrl_unregister(struct snd_ctrl_data *snd_data)
{
	...
	mutex_lock(&list_mutex);
	if (unlikely(!find_ctrl_data(snd_data->name))) {
		pr_err("%s ctrl data does not exist\n", snd_data->name);
		mutex_unlock(&list_mutex);
		return;
	}
	...
	if (!list_empty(&ctrl_list) && snd_ctrl_data_global(snd_data)) {
		 ...
		 */
		ctrl_data = NULL;
		pr_info("No available ctrl data yet\n");
	}
	mutex_unlock(&list_mutex);
}
 ...
 * data. Returns -EINVAL[-ENOENT] on failure or 0 on success.
 */
static inline int snd_ctrl_data_fill(struct snd_ctrl_data *snd_data)
{
	int ret = -EINVAL, i;

	/* Return early if def_data is not specified */
	if (IS_ERR_OR_NULL(def_data))
		return -ENOENT;
 ...
 *
 * This function expects sound line data to be passed in a 2-byte u32 array:
...
static inline int __devinit snd_ctrl_parse_dt(struct device_node *node)
{
	 ...
	 * @id: id of a sound line to store the values into.
	 *
 ...
 * point that it is adapted to platform data implementation. There a simple
...
static int __devinit snd_ctrl_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(def_data)) {
		pr_err("Unable to allocate memory for default control data\n");
		return -ENOMEM;
	}
