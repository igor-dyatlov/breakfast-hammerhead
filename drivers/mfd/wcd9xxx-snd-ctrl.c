...
static inline bool snd_ctrl_data_expected(struct snd_ctrl_data *snd_data)
{
	...
	return !strncmp(stats->data.name, snd_data->name, CTRL_NAME_LEN);
}
...
{
	return !(IS_ERR_OR_NULL(ctrl_data) ||
		 IS_ERR_OR_NULL(ctrl_data->name) ||
		 IS_ERR_OR_NULL(ctrl_data->codec) ||
		 IS_ERR_OR_NULL(ctrl_data->read) ||
		(IS_ERR_OR_NULL(ctrl_data->write) && !read_only));
}
...
/**
 * apply_gains() - write default gains from local data to a passed control data.
 * @snd_data: pointer to sound control data.
 */
...
static ssize_t show_##name##_gain(struct kobject *kobj,			\
				  ...
				  char *buf)				\
{									\
	if (!snd_glbl_data_ready(__RL) || !line_present(name))		\
...
{									\
	int ret, val;							\
									\
	if (!snd_glbl_data_ready(__WL) || !line_present(name))		\
		return -ENOENT;						\
									\
